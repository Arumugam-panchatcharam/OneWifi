...
webconfig_error_t decode_em_policy_object(const cJSON *em_cfg, em_config_t *em_config)
{
    ...
    if (cJSON_IsArray(traffic_sep_array) == false) {
        wifi_util_error_print(WIFI_WEBCONFIG,
            "%s:%d:[TRAFFIC_SEP] Traffic Separation Policy object not present\n", __func__, __LINE__);
        return webconfig_error_decode;
    }
    ...
    for (int i = 0; i < em_config->traffic_separation_policy.ssids_num; i++) {
        ...
        wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: [TRAFFIC_SEP] Traffic Separation SSID: %s, VLAN: %d %d\n", __func__, __LINE__,
                param->valuestring, em_config->traffic_separation_policy.ssids[i].vlan_id);

        strncpy(em_config->traffic_separation_policy.ssids[i].ssid, param->valuestring,
