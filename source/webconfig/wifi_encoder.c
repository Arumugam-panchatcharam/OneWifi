...
webconfig_error_t encode_em_config_object(const em_config_t *em_config, cJSON *emconfig_obj)
{
    ...
    for (int i = 0; i < em_config->traffic_separation_policy.ssids_num; i++) {
	    ...
	    wifi_util_dbg_print(WIFI_WEBCONFIG, "%s:%d: [TRAFFIC_SEP] Traffic Separation SSID: %s, VLAN: %d\n", __func__, __LINE__, em_config->traffic_separation_policy.ssids[i].ssid,
